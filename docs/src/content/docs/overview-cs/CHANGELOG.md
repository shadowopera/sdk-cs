...
- Added `AtlasOptions.WithVariant(key, variant)` for non-default variant selection.
- MinMax: Added `Duration` support to `MinMax<T>`, including `Sample` extension methods for .NET and Unity.
